...
raw = fetch_pob_from_url('https://pobb.in/QxEmf-NPx1qM')
...
print('\n=== PLAYER STATS (SAVED IN POB) ===')
for stat in root.findall('.//PlayerStat'):
    print(f"{stat.attrib.get('stat')}: {stat.attrib.get('value')}")
...
if tree is not None:
    ...
    if spec is not None:
        ...
        print('Node IDs count:', len(nodes.split(',')))
